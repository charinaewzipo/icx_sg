    ...
    jQuery.rpt = {
        ...
        getgroup: function () {
            ...
            $.ajax({
                ...
                'success': function (data) {
                    ...
                    $('[name=group_id]').text('').append(option);
        ...
        agent_performance_report: function () {
            ...
            $.ajax({
                'url': url,
                'data': {
                    'action': 'agent_performance_report',
                    'data': formtojson
                },
                'dataType': 'html',
                'type': 'POST',
                ...
                'success': function (data) {
                    ...
                    for (i = 0; i < result.length; i++) {
                        ...
                        txt += "<tr><td style='vertical-align:middle; text-align:center' >&nbsp;" + result[i].gname + "&nbsp;</td>" +
                            ...
                            "<td style='vertical-align:middle; text-align:center'  >" + result[i].lname + "</td>" +
        ...
        agent_performance_export() {
            ...
            var formtojson = JSON.stringify($('form').serializeObject());
            $.post(url, {
                "action": "agent_performance_report_export",
                "data": formtojson
            }, function (result) {
        ...
        confirm_call_report: function () {
            ...
            $.ajax({
                ...
                'success': function (data) {
                    ...
                    var result = eval('(' + data + ')');
                    var $table = $('#confirm-call-table tbody');
                    $table.find('tr').remove();
                    var i = 0,
                        txt = "";

                    for (i = 0; i < result.length; i++) {
                        txt += "<tr><td style='vertical-align:middle; text-align:center' >&nbsp;" + result[i].appid + "&nbsp;</td>" +
                            "<td style='vertical-align:middle; text-align:center' >" + result[i].cname + "</td>" +
                            "<td style='vertical-align:middle; text-align:center' >" + result[i].aname + "</td>" +
                            "<td style='vertical-align:middle; text-align:center'  >" + result[i].sdate + "</td>" +
                            "<td style='vertical-align:middle; text-align:center' >" + result[i].adate + "</td>" +
                            "<td style='vertical-align:middle; text-align:center' >" + result[i].udate + "</td>" +
                            "<td style='vertical-align:middle; text-align:center' >" + result[i].astatus + "</td>" +
                            "<td style='vertical-align:middle; text-align:center' >" + result[i].confirmid + "</td>" +
                            "</tr>";
                    }
                    ...
                    if (i == 0) {
                        $row = $("<tr id='nodata'><td colspan='8' class='text-center'>&nbsp; No data found &nbsp;</td></tr>")
                        $row.appendTo($table);
                    } else {
                        var $table = $('#confirm-call-table tfoot');
                        $table.find('tr').remove();
                        var s = "s";
                        if (i < 1) {
                            s = "";
                        }
                        $addRow = $("<tr ><td colspan='7'  style='border-top: 1px solid #EAEAEA' ><small> Total " + i + " Record" + s + " </small></td></tr>");
                        $addRow.appendTo($table);
                    }
        ...
        confirm_call_export() {
            ...
            var formtojson = JSON.stringify($('form').serializeObject());
            $.post(url, {
                "action": "confirm_call_report_export",
                "data": formtojson
            }, function (result) {
                var response = eval('(' + result + ')');
                if (response.result == "success") {
                    window.location = "dw.php?p=/temp/call_confirm.xlsx&o=y";
                }
            });
                ...
                'success': function (data) {
                    ...
                    for (i = 0; i < result.length; i++) {
                        ...
                        txt += "<tr><td style='vertical-align:middle;' >" + result[i].lanme + "</td>" +
                            ...
                            "<td >" + result[i].policyapp + "</td>" +
                            "<td >" + result[i].policyinf + "</td>" +
                            "</tr>";
                    }
                    ...
                    if (i == 0) {
                        $row = $("<tr id='nodata'><td colspan='26' class='text-center'>&nbsp; No data found &nbsp;</td></tr>")
                        $row.appendTo($table);
                    } else {
                        ...
                        $addRow = $("<tr ><td colspan='26'  style='border-top: 1px solid #EAEAEA' ><small> Total " + i + " Record" + s + " </small></td></tr>");
                        $addRow.appendTo($table);
                    }
        ...
        list_conversion_export() {
            ...
            var formtojson = JSON.stringify($('form').serializeObject());
            $.post(url, {
                "action": "list_conversion_report_export",
                "data": formtojson
            }, function (result) {
                var response = eval('(' + result + ')');
                if (response.result == "success") {
        ...
        _countPerson: function ($id, sizeOful) {
            ...
            switch (sizeOful.length) {
                case 0:
                    ...
                    break;
                case 1:
                    $id.text('').text(sizeOful.length + ' person');
                    break;
                case 2:
                    $id.text('').text(sizeOful.length + ' persons');
                    break;
                default:
                    $id.text('').text(sizeOful.length + ' persons');
