(function ($) {
  ...
  jQuery.rpt = {
    ...
    getleadbycmp: function () {
      var formtojson = JSON.stringify($("form").serializeObject());
      // Get the selected campaign_ids from the campaign_id dropdown
      var selectedCampaignIds = $("#campaign_id").val();
      ...
      $.ajax({
        ...
        data: {
          action: "getleadbycmp",
          data: formtojson,
          // Pass the selected campaign_ids to the server
        ...
        success: function (data) {
          ...
          for (i = 0; i < resultFiltered.length; i++) {
            option +=
              "<option value='" +
              resultFiltered[i].import_id +
              "'>" +
              resultFiltered[i].list_name +
              "</option>";
          }
          // Update the options of the campaign_id_lead_list dropdown
          $("[name=campaign_id_lead_list]").text("").append(option);
        },
      });
    },
    getstatus: function () {
      ...
      $.ajax({
        ...
        success: function (data) {
          var result = eval("(" + data + ")");
          //group location
          console.log("getstatus=>555", result);
          var option = "<option value=''> &nbsp; </option>";
          for (i = 0; i < result.length; i++) {
            option +=
              "<option value='" +
              result[i].wrapup_code +
              "'>" +
              result[i].status +
              "</option>";
          }

          $("[name=status_list]").text("").append(option);
        },
      }); //end ajax
    },
        ...
        success: function (data) {
          ...
          for (i = 0; i < result.length; i++) {
            txt +=
              ...
              "</td>" +
              "<td style='vertical-align:middle; text-align:center' >" +
              result[i].transfertotal +
              "</td>" +
              "<td style='vertical-align:middle; text-align:center' >" +
              result[i].revoketotal +
              "</td>" +
              "<td style='vertical-align:middle; text-align:center' >" +
              result[i].lonhand +
              "</td>" +
              ...
              "</td>" +
              "<td style='vertical-align:middle; text-align:center' >" +
              result[i].callbadlist +
              "</td>" +
              "<td style='vertical-align:middle; text-align:center' >" +
              result[i].closedsales +
              "</td>" +
              "<td style='vertical-align:middle; text-align:center' >" +
              result[i].unclosedsales +
              "</td>" +
          ...
          if (i == 0) {
            $row = $(
              "<tr id='nodata'><td colspan='17' class='text-center'>&nbsp; No data found &nbsp;</td></tr>"
            );
            $row.appendTo($table);
          } else {
            ...
            $addRow = $(
              "<tr ><td colspan='17'  style='border-top: 1px solid #EAEAEA' ><small> Total " +
                i +
                " Record" +
                s +
                " </small></td></tr>"
    ...
    confirm_call_report: function () {
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          if (i == 0) {
            $row = $(
              "<tr id='nodata'><td colspan='7' class='text-center'>&nbsp; No data found &nbsp;</td></tr>"
            );
            $row.appendTo($table);
          } else {
            ...
            $addRow = $(
              "<tr ><td colspan='7'  style='border-top: 1px solid #EAEAEA' ><small> Total " +
                i +
                " Record" +
                s +
                " </small></td></tr>"
    ...
    list_conversion_report: function () {
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          if (i == 0) {
            $row = $(
              "<tr id='nodata'><td colspan='26' class='text-center'>&nbsp; No data found &nbsp;</td></tr>"
            );
            $row.appendTo($table);
          } else {
            ...
            $addRow = $(
              "<tr ><td colspan='26'  style='border-top: 1px solid #EAEAEA' ><small> Total " +
                i +
                " Record" +
                s +
                " </small></td></tr>"
    ...
    _rtl: function (alluser) {
      ...
      if (alluser.length != 0) {
        for (i = 0; i < alluser.length; i++) {
          li +=
            '<li class="stack-ltr"><input type="checkbox" value=' +
            alluser[i].aid +
            ' class="list-user"> ' +
            alluser[i].fname +
            " " +
    ...
    _ltr: function (member, nomember) {
      ...
      if (member.length != 0) {
        for (i = 0; i < member.length; i++) {
          li +=
            '<li class="stack-rtl"><input type="checkbox" value=' +
            member[i].aid +
            ' class="list-member"> ' +
            member[i].fname +
            " " +
