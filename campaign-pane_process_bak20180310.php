...
<?php
   	...
   	if(isset($_POST['action'])){ 
         ...
         switch( $_POST['action']){
			...
			//save -> insert , update
			case "check" : check_cmpname(); break;
			...
			case "wrapup"  : wrapup(); break; //query wrapup
		 }
	}
	...
	function detail(){
		...
		$res = $dbconn->createConn();
	 	if($res==404){
  			...
  			$sql = " SELECT campaign_id,campaign_name,campaign_detail, app_id , maximum_call , start_date,end_date, c.status, st.status_detail, st.status_color , c.script_id,s.script_detail, ". 
					...
					//check is save again ( fix user alway save with same data );
				 	...
				 	if( $tmp['cmpuniqueid'] != "" ){
						...
						$uniqueid = $tmp['cmpuniqueid'];
					 	...
					 	//insert new campaign
					   $sql = " INSERT INTO t_campaign( campaign_uniqueid , campaign_name , campaign_detail, is_expire , start_date,end_date,status , script_id, create_user, create_date ) VALUES ( ".
					      			" ".dbformat( $uniqueid )." ".
