...
$SQL = "select import_id from t_calllist_agent where (last_wrapup_dt BETWEEN '$startdate' and '$enddate') and campaign_id = '1'  GROUP BY import_id DESC" ;
...
{ 
	...
	$SQL_count =  mysql_query("  select sum(PolicyPremium) FROM app_mt where (Sale_Date BETWEEN '$startdate1' and '$enddate1') and campaign_id = '1' and import_id = '$import_id' and PaymentFrequency = 'Monthly'  ");
	$list_submit_m = mysql_result($SQL_count, 0);
	
	$SQL_count =  mysql_query("  select sum(PolicyPremium) FROM app_mt where (Sale_Date BETWEEN '$startdate1' and '$enddate1') and campaign_id = '1' and import_id = '$import_id' and PaymentFrequency <> 'Monthly' ");
	$list_submit_a= mysql_result($SQL_count, 0);
	
	$list_submit = ($list_submit_m*12)+$list_submit_a;
	
	$SQL_count =  mysql_query("  select sum(PolicyPremium) FROM app_mt where (Approved_Date BETWEEN '$startdate1' and '$enddate1') and campaign_id = '1' and import_id = '$import_id' and PaymentFrequency = 'Monthly' and AppStatus = 'Approved' ");
	$list_approved_m = mysql_result($SQL_count, 0);
	
	$SQL_count =  mysql_query("  select sum(PolicyPremium) FROM app_mt where (Approved_Date BETWEEN '$startdate1' and '$enddate1') and campaign_id = '1' and import_id = '$import_id' and PaymentFrequency <> 'Monthly' and AppStatus = 'Approved' ");
	$list_approved_a = mysql_result($SQL_count, 0);
	
	$list_approved = ($list_approved_m*12)+$list_approved_a;
