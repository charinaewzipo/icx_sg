...
<?php
  ...
  wlog("normal query : ".$sql);
	...
	function save_wrapup(){
				...
				$sql = "SELECT remove_list FROM t_wrapup_code WHERE wrapup_code = ".dbNumberFormat($tmp['wrapup3'])."  ";
				...
				//check wrapup option 
				$option = null;
				$sql = "SELECT option_id  FROM t_wrapup_code WHERE wrapup_code = ".dbNumberFormat($tmp['wrapup3'])."  ";
			    $result = $dbconn->executeQuery($sql); 
				if($rs=mysql_fetch_array($result)){   
					$option = $rs['option_id']; 
				}
