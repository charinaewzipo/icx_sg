...
";
	...
	switch ($lv) {
		...
		case 4:
			...
			break;
		case 5:
			$sql .= " AND  !(qcqa.create_date IS NULL AND qcqa.QAStatus is not null)";
			break;
		case 3:
			$sql .= " AND  !(qcqa.create_date IS NULL AND qcqa.QAStatus is not null)";
			break;
	...
	/*
		if(  $lv == 1){
		  ...
		  */
	if ($tmp['search_cust'] != "") {
		$condition = $condition . " AND app.agent_id = " . dbformat($tmp['search_cust']) . " ";
	}
	if ($tmp['search_agent'] != "") {
		$condition = $condition . " AND app.campaign_id = " . dbformat($tmp['search_agent']) . " ";
	}
	// if ($tmp['search_quono'] != "") {
	...
	if ($tmp['search_cred'] != "" & $tmp['search_endd'] != "") {
		$condition = $condition . " AND app.last_wrapup_dt BETWEEN '" . $tmp['search_cred'] . " 00:00' AND  '" . $tmp['search_endd'] . " 23:59'";
	}
	...
	wlog('[applicationVoice-pane_process][query] query : ' . $sql);
	...
	while ($rs = mysqli_fetch_array($result)) {
		...
		$data[$count] = array(
			"id"  => nullToEmpty($rs['app_id']),
			//"quono"  => nullToEmpty($rs['quotation']),
			"cname"  => nullToEmpty($rs['owner']),
			...
			"qcstatus"  => nullToEmpty($rs['agentUpdateBy']),
			"qastatus"  => nullToEmpty($rs['qa_status'])
		);
		$count++;
	}
	if ($count == 0) {
	...
	$sql = "SELECT COUNT(app.app_id) AS total " .
		"FROM t_aig_app_non_sale app LEFT OUTER JOIN  t_agents  agent ON app.agent_id = agent.agent_id";
