...
function updatelist()
{
	...
	$sql = "UPDATE t_import_list SET " .
		...
		" is_count = " . $is_count . ", " .
		" genesys_campaign_name = " . dbformat($tmp['genesys_campaign_name']) . ", " .
		" genesys_campaign_id = " . dbformat($tmp['genesys_campaign_id']) . ", " .
		" genesys_list_name = " . dbformat($tmp['genesys_list_name']) . ", " .
		" genesys_list_id = " . dbformat($tmp['genesys_list_id']) . ", " .
		" genesys_queue_name = " . dbformat($tmp['genesys_queue_name']) . ", " .
		" genesys_queue_id = " . dbformat($tmp['genesys_queue_id']) . ", " .
...
function deleteList()
{
	...
	$sql = "UPDATE t_calllist_agent SET status = 0, last_wrapup_detail = 'Delete List', last_wrapup_option_id = 3 WHERE calllist_id IN ( SELECT calllist_id FROM t_calllist WHERE import_id  = " . dbNumberFormat($tmp['impid']) . " ) ";
	wlog("[callList_process][deleteList] update AGENT CALLLIST sql : " . $sql);
	$result = $dbconn->executeUpdate($sql);
	...
	$sql = "UPDATE t_import_list SET status = 0 WHERE import_id = " . dbNumberFormat($tmp['impid']);
	wlog("[callList_process][deleteList] upadte IMPORT LIST sql : " . $sql);
	$result = $dbconn->executeUpdate($sql);
...
function detail()
{
	...
	$sql = " SELECT import_id ,list_name,list_detail,list_comment,l.status,file_name,file_size,file_type, total_records,bad_list, " .
		...
		" is_count,genesys_campaign_name,genesys_campaign_id,genesys_list_name,genesys_list_id,genesys_queue_name,genesys_queue_id " .
	...
	if ($rs = mysqli_fetch_array($result)) {
		$tmp2 = array(
			...
			"is_count" => nullToEmpty($rs['is_count']),
			"genesys_campaign_name" => nullToEmpty($rs['genesys_campaign_name']),
			"genesys_campaign_id" => nullToEmpty($rs['genesys_campaign_id']),
			"genesys_list_name" => nullToEmpty($rs['genesys_list_name']),
			"genesys_list_id" => nullToEmpty($rs['genesys_list_id']),
			"genesys_queue_name" => nullToEmpty($rs['genesys_queue_name']),
...
function removeTemp()
{
	...
	while ($rs = mysqli_fetch_array($result)) {
		...
		$sql = "DROP TABLE tmp_table" . $rs['import_key'];
...
function mappingfield()
{
	...
	for ($i = 0; $i < $size; $i++) {
		if ($i == 0) {
			$sql = $sql . ' length(' . $phonefield[$i] . ') not in (9,10) ';
		} else {
			$sql = $sql . ' AND length(' . $phonefield[$i] . ') not in (9,10) ';
		}
	}
	$sql = $sql . ")";
	...
	if (!empty($_FILES)) {
		...
		if ($_FILES["file"]["error"] > 0) {
			...
			switch ($_FILES["file"]["error"]) {
				case 1:
					wlog("[callList_process][upload] Error : The uploaded file exceeds the upload_max_filesize directive in php.ini");
					break;
				case 2:
					wlog("[callList_process][upload] Error :The uploaded file exceeds the MAX_FILE_SIZE directive that was specified in the HTML form");
					break;
				case 3:
					wlog("[callList_process][upload] Error :  The uploaded file was only partially uploaded");
					break;
				case 4:
					wlog("[callList_process][upload] Error :  No file was uploaded");
					break;
				case 5:
					wlog("[callList_process][upload] Error : Missing a temporary folder");
					break;
				case 6:
					wlog("[callList_process][upload] Error : Failed to write file to disk");
					break;
				case 7:
					wlog("[callList_process][upload] Error : File upload stopped by extension");
					break;
				default:
					wlog("[callList_process][upload] Error : Unknown upload error");
					break;
			}
		} else {
...
function genesysExport()
{
	...
	for ($round = 0; $round <= $allpage; $round++) {
		...
		wlog("[callList_process][exportgenesys1test] sql : " . $sql);
		$result = $dbconn->executeQuery($sql);
		if (mysqli_num_rows($result) <= 0) break;
		$data = array();
		wlog("[callList_process][echocount6] checkcount or temp :data " . mysqli_num_rows($result));
		while ($rs = mysqli_fetch_assoc($result)) {
			...
			array_push($data, $tmp);
			$count++;
			wlog("[callList_process][echocount] checkcount or temp : " . $count .  print_r($data, true));
		}
		...
		if (is_array($return_data) && count($return_data) > 0) {
			...
			wlog("[callList_process][exportgenesys2] insertLog : " . $insert_q);
