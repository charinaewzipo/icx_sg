...
<body>
  ...
  $SQL = "select  campaign_name,CONCAT(agents.first_name,\" " .  "\",agents.last_name) as tsr_name,app.calllist_id " .
    "from t_aig_app_non_sale app " .
    "left join  t_campaign c on app.campaign_id = c.campaign_id " .
    "left join t_agents agents on app.agent_id = agents.agent_id " .
    "where app.app_id = '$id'";
  $result = mysqli_query($Conn, $SQL) or die("ไม่สามารถเรียกดูข้อมูลได้");
  while ($row = mysqli_fetch_array($result)) {

    $campaign_name = $row["campaign_name"];
    $tsr_fullname = $row["tsr_name"];
        ...
        <!-- <div>
          <fieldset id="form-content">
            ...
            <div style="padding: 10px 50px 10px 50px ;  "><input type="checkbox" id="GIVEN_BENAFICIARY" name="GIVEN_BENAFICIARY" value="1" <?php echo $GIVEN_BENAFICIARY_GIVEN_BENAFICIARY; ?>>
              <label for="vehicle1"> ระบุเป็น ทายาทโดยธรรม &nbsp;&nbsp;<span style="color:red">* ถ้าระบุ ไม่ต้องทำการกรอกข้อมูลผู้รับผลประโยชน์</label></span><br>
            </div>
            <div>
          ...
          <fieldset id="form-content">
            ...
            <div class="table">
              ...
              <div style="padding: 10px 50px 10px 50px ;  "><input type="checkbox" id="APP_CONSENT" name="APP_CONSENT" value="Y" <?php echo $APP_CONSENT; ?>>
                <label for="vehicle1"> ไม่ยินยอมให้เปิดเผยข้อมูล &nbsp;&nbsp;<span style="color:red">* กรณีลูกค้าไม่ให้ความยินยอมส่งข้อมูลสรรพากร ลดหย่อนภาษี</label></span><br>
              </div>
        ...
        <div>
          <fieldset id="form-content">
            ...
            <div class="table">
              ...
              <div class="table-content">
                <?php
                $voice_sql = "select app_id,voice_file_name,voice_create_dt from t_aig_app_non_sale where app_id = '" . $id . "'";
                $vocie_result = mysqli_query($Conn, $voice_sql) or die("ไม่สามารถเรียกดูข้อมูลได้");
                $v_index = 1;
                while ($voice_row = mysqli_fetch_array($vocie_result)) {
                  $created_date = new DateTime($voice_row['voice_create_dt'], new DateTimeZone('Asia/Bangkok'));
                  $voice_file_name = $voice_row['voice_file_name'];
                  // $voice_path = file_get_contents("\\192.168.30.5\Voice Recording\Auto Download Voice Non Sale\\" . $voice_file_name);
                ?>
                  <div class="table-row">
                    <!-- <div class="table-data"><?php echo $voice_path; ?></div> -->
                    ...
                    <div class="table-data"><?php echo  $voice_file_name; ?></div>
                    <!-- <audio controls>
                      <source src="file://///192.168.30.5/Voice%20Recording/Auto%20Download%20Voice%20Non%20Sale/1_Bandit%20Sapcharoen_Central%20-%20Sabaijai.mp3" type="audio/mpeg">
                    </audio> -->
