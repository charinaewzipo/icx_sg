...
<body>
  ...
  $SQL = "select  a.*,b.first_name,b.last_name,c.plancode,c.campaign_name,c.max_insured,c.max_beneficiary,c.campaign_test_code,c.url_billing_payment,c.url_online_payment,c.check_bin from t_aig_app a left outer join t_agents b ON a.agent_id=b.agent_id left outer join t_campaign c ON a.campaign_id=c.campaign_id where a.id = '$id'";
  ...
  while ($row = mysqli_fetch_array($result)) {
    ...
    $max_insured = $row["max_insured"];
    $max_beneficiary = $row["max_beneficiary"];
    $campaign_test_code = $row["campaign_test_code"];
    $url_billing_payment = $row["url_billing_payment"];
    $url_online_payment = $row["url_online_payment"];
    ...
    if ($row["INSURED_DOB3"] != "") {
      $IN3_DOB = $row["INSURED_DOB3"];
      if (!in_array($lv, [3, 7])) {
        $IN3_DOB_val = "XX/XX/" . substr($IN3_DOB, -4);
        $IN3_xxx_DOB = "XX/XX/" . (intval(substr($IN3_DOB, -4)) + 543);
      } else {
        $IN3_DOB_val = $IN3_DOB;
        $IN3_xxx_DOB = substr($IN3_DOB, 0, 6) . (intval(substr($IN3_DOB, -4)) + 543);
      }
    }

    if ($row["INSURED_DOB4"] != "") {
      $IN4_DOB = $row["INSURED_DOB4"];
      if (!in_array($lv, [3, 7])) {
        $IN4_DOB_val = "XX/XX/" . substr($IN4_DOB, -4);
        $IN4_xxx_DOB = "XX/XX/" . (intval(substr($IN4_DOB, -4)) + 543);
      } else {
        $IN4_DOB_val = $IN4_DOB;
        $IN4_xxx_DOB = substr($IN4_DOB, 0, 6) . (intval(substr($IN4_DOB, -4)) + 543);
      }
    }

    if ($row["INSURED_DOB5"] != "") {
      $IN5_DOB = $row["INSURED_DOB5"];
      if (!in_array($lv, [3, 7])) {
        $IN5_DOB_val = "XX/XX/" . substr($IN5_DOB, -4);
        $IN5_xxx_DOB = "XX/XX/" . (intval(substr($IN5_DOB, -4)) + 543);
      } else {
        $IN5_DOB_val = $IN5_DOB;
        $IN5_xxx_DOB = substr($IN5_DOB, 0, 6) . (intval(substr($IN5_DOB, -4)) + 543);
      }
    }
  ...
  ?>
    ...
    <div id="content">
      ...
      <form id="application">
        ...
        <input type="hidden" name="OLD_IN3_DOB" value="<?php echo $row["INSURED_DOB3"]; ?>">
        <input type="hidden" name="OLD_IN4_DOB" value="<?php echo $row["INSURED_DOB4"]; ?>">
        <input type="hidden" name="OLD_IN5_DOB" value="<?php echo $row["INSURED_DOB5"]; ?>">
        <input type="hidden" name="campaign_id" id="campaign_id" value="<?php echo $row["campaign_id"]; ?>">
        <input type="hidden" name="check_bin" id="check_bin" value="<?php echo $row["check_bin"]; ?>">
          ...
          <fieldset id="form-content">
            <table id="table-form">
              <tr>
                ...
                <td>
                  <input type="text" name="Effective_Date_V" size="10" value="<?php echo ($row["effective_date"] == "") ? "" : date('Y-m-d', strtotime($row["effective_date"])); ?>"/>
                  <input type="hidden" name="Effective_Date" value="<?php echo $row["effective_date"]; ?>">
                </td>
              </tr>
            </table>
          </fieldset>
          ...
          <fieldset id="form-content">
            ...
            <table id="table-form">
              ...
              <tr>
                ...
                <th>&nbsp;</th>
                <td>สถานภาพ : <span style="color:red">*</span></td>
                <td>
                  <select name="INSURED_STATUS">
                    <option value="<?php echo $row["insured_status"]; ?>">
                      <?php echo $row["insured_status"]; ?>
                    </option>
              ...
              <tr>
                <td>ที่อยู่ 1 : <span style="color:red">*</span></td>
                <td><input type="text" name="ADDRESSNO1" size="45" value="<?php echo $row["ADDRESSNO1"]; ?>"/></td>
              </tr>
              <tr>
                <td>ที่อยู่ 2 : </td>
                <td><input type="text" name="BUILDING1" size="45" value="<?php echo $row["BUILDING1"]; ?>"/></td>
              </tr>
              <tr>
              ...
              <td>แขวง/ตำบล : <span style="color:red">*</span></td>
                <td><select name="SUB_DISTRICT1" id="Sub_district1"
                    onchange="show_ZIPCODE_Value(this.value,document.getElementById('district1').value,document.getElementById('province1').value)">
                  ...
                  <td>
                  <input type="text" name="ZIPCODE1" id="zipcode1" value="<?php echo $row["ZIPCODE1"]; ?>" size="15"/>
                  </td>
              </tr>
              <tr>
              ...
              <tr>
                ...
                <td <?php if ($campaign_test_code != 'SMK') {
                      echo $hide;
                    } ?>>
                <?php
                if ($row["smoke"] == '1') {
                ...
                ?>
                    <input type="checkbox" id="SMOKE" name="SMOKE" value="1" <?php echo $CHK_SMOKE; ?> ><label for="vehicle1"> สูบบุหรี่ </label>
                </td>
              </tr>
        ...
        <div>
          <!-- <fieldset id="form-content">
            ...
            <table id="table-form">
              <tr>
                ...
                <td>
                  <select name="PRODUCT_NAME" onchange="getPremiumD2C()">
                    <option value="<?php echo $row["PRODUCT_NAME"]; ?>">
                      <?php echo $row["PRODUCT_NAME"]; ?>
                    </option>
                    <?php
                    $strSQL = "SELECT DISTINCT PRODUCTNAME_TH FROM t_aig_doublecare_product WHERE campaign_id = '$campaign_id'";
                    $objQuery = mysqli_query($Conn, $strSQL);
                    while ($objResuut = mysqli_fetch_array($objQuery)) {
                    ?>
                      <option value="<?php echo $objResuut["PRODUCTNAME_TH"]; ?>">
                        <?php echo $objResuut["PRODUCTNAME_TH"]; ?>
                      </option>
                    <?php
                    }
                    ?>
              ...
              <tr>
                ...
                <td><select name="COVERAGE_NAME" onchange="getPremiumD2C()">
                    <option value="<?php echo $row["COVERAGE_NAME"]; ?>">
                      <?php echo $row["COVERAGE_NAME"]; ?>
                    </option>
                    <?php
                    $strSQL = "SELECT DISTINCT PLAN FROM t_aig_doublecare_product WHERE campaign_id = '$campaign_id'";
                    $objQuery = mysqli_query($Conn, $strSQL);
                    while ($objResuut = mysqli_fetch_array($objQuery)) {
                    ?>
                      <option value="<?php echo $objResuut["PLAN"]; ?>">
                        <?php echo $objResuut["PLAN"]; ?>
                      </option>
                    <?php
                    }
                    ?>
                  </select>&nbsp;
                  Plan Layer Code: <input type="text" name="PLAN_LAYER_CODE" id="PLAN_LAYER_CODE" readonly></td>
              </tr>
              <td>การชำระเงิน : <span style="color:red">*</span></td>
              <td><select name="PAYMENTFREQUENCY" onchange="getPremiumD2C()">
              ...
              <tr>
                ...
                <td><input type="text" name="INSTALMENT_PREMIUM" id="INSTALMENT_PREMIUM" maxlength="15" value="<?php echo $row["INSTALMENT_PREMIUM"]; ?>" readonly /> บาท&nbsp;
                  ยอดที่ใช้ตัด Payment : <input type="text" name="INSTALMENT_COPY" id="INSTALMENT_COPY" maxlength="15" value="" readonly /> บาท
                </td>
              </tr>
            </table><br /> -->
              ...
              <table id="table-form">
                ...
                <tr>
                  ...
                  <td>
                    ...
                    <input id="datepicker2" type="text" name="INSURED_DOB2_V" maxlength="10" value="<?php echo $IN2_xxx_DOB; ?>" />
                  </td>
                  <th>&nbsp;</th>
                  <td>ความสัมพันธ์ : </td>
                  <td><select name="INSURED_RELATIONSHIP2">
                      ...
                      <?php
                      $strSQL = "SELECT * FROM t_aig_pancode where type = 'relationship' and code in ('S','O','D') ORDER BY id ASC";
                      $objQuery = mysqli_query($Conn, $strSQL);
                      while ($objResuut = mysqli_fetch_array($objQuery)) {
                      ?>
                        <option value="<?php echo $objResuut["name"]; ?>">
                ...
                <tr>
                  <td>เพศ : </td>
                  <td><select name="INSURED_SEX2">
                      <option value="<?php echo $row["INSURED_SEX2"]; ?>">
                        <?php echo $row["INSURED_SEX2"]; ?>
                      </option>
                      <option value="ชาย">ชาย</option>
                      <option value="หญิง">หญิง</option>
                    </select></td>
          ...
          <!--Insure 3-->
          <!-- <div <?php if ($max_insured < 3) {
                      echo $hide;
                    } ?>>
              <div>
                <h2>ผู้เอาประกันที่ 3</h2>
              </div>
              <table id="table-form">
                <tr>
                  <td>คำนำหน้า : </td>
                ...
                <tr>
                  <td>วัน/เดือน/ปี เกิด : </td>
                  <td>
                    <input type="hidden" name="INSURED_DOB3" maxlength="10" value="<?php echo $IN3_DOB_val; ?>" />
                    <input type="text" name="INSURED_DOB3_V" maxlength="10" value="<?php echo $IN3_xxx_DOB; ?>" />
                  </td>
                  <th>&nbsp;</th>
                  <td>ความสัมพันธ์ : </td>
                ...
                <tr>
                  <td>เพศ : </td>
                  <td><select name="INSURED_SEX3">
                      <option value="<?php echo $row["INSURED_SEX3"]; ?>">
                        <?php echo $row["INSURED_SEX3"]; ?>
                      </option>
                      <option value="ชาย">ชาย</option>
                      <option value="หญิง">หญิง</option>
                    </select></td>
          ...
          <!--Insure 4-->
          <!-- <div <?php if ($max_insured < 4) {
                      echo $hide;
                    } ?>>
              <div>
                <h2>ผู้เอาประกันที่ 4</h2>
              </div>
              <table id="table-form">
                <tr>
                  <td>คำนำหน้า : </td>
                ...
                <tr>
                  <td>วัน/เดือน/ปี เกิด : </td>
                  <td>
                    <input type="hidden" name="INSURED_DOB4" maxlength="10" value="<?php echo $IN4_DOB_val; ?>" />
                    <input type="text" name="INSURED_DOB4_V" maxlength="10" value="<?php echo $IN4_xxx_DOB; ?>" />
                  </td>
                  <th>&nbsp;</th>
                  <td>ความสัมพันธ์ : </td>
                ...
                <tr>
                  <td>เพศ : </td>
                  <td><select name="INSURED_SEX4">
                      <option value="<?php echo $row["INSURED_SEX4"]; ?>">
                        <?php echo $row["INSURED_SEX4"]; ?>
                      </option>
                      <option value="ชาย">ชาย</option>
                      <option value="หญิง">หญิง</option>
                    </select></td>
          ...
          <!--Insure 5-->
          <!-- <div <?php if ($max_insured < 5) {
                      echo $hide;
                    } ?>>
              <div>
                <h2>ผู้เอาประกันที่ 5</h2>
              </div>
              <table id="table-form">
                <tr>
                  <td>คำนำหน้า : </td>
                  <td>
                    <select name="INSURED_TITLE5">
                      <option value="<?php echo $row["INSURED_TITLE5"]; ?>"><?php echo $row["INSURED_TITLE5"]; ?></option>
                      <?php
                      $strSQL = "SELECT * FROM t_aig_pancode where type = 'titlename' ORDER BY id ASC";
                      $objQuery = mysqli_query($Conn, $strSQL);
                      while ($objResuut = mysqli_fetch_array($objQuery)) {
                ...
                <tr>
                  <td>วัน/เดือน/ปี เกิด : </td>
                  <td>
                    <input type="hidden" name="INSURED_DOB5" maxlength="10" value="<?php echo $IN5_DOB_val; ?>" />
                    <input type="text" name="INSURED_DOB5_V" maxlength="10" value="<?php echo $IN5_xxx_DOB; ?>" />
                  </td>
                  <th>&nbsp;</th>
                  <td>ความสัมพันธ์ : </td>
                ...
                <tr>
                  <td>เพศ : </td>
                  <td><select name="INSURED_SEX5">
                      <option value="<?php echo $row["INSURED_SEX5"]; ?>">
                        <?php echo $row["INSURED_SEX5"]; ?>
                      </option>
                      <option value="ชาย">ชาย</option>
                      <option value="หญิง">หญิง</option>
                    </select></td>
        ...
        <!-- <div>
          <fieldset id="form-content">
            ...
            <div <?php if ($max_beneficiary < 1) {
              ...
              <table id="table-form">
                ...
                <tr>
                  ...
                  <td><input type="number" name="BENEFICIARY_BENEFIT1" maxlength="3" size="8" value="<?php echo $row["BENEFICIARY_BENEFIT1"]; ?>" /></td>
                </tr>
                <tr>
                  <td>เพศ : </td>
            ...
            <div <?php if ($max_beneficiary < 2) {
              ...
              <table id="table-form">
                ...
                <tr>
                  ...
                  <th>&nbsp;</th>
                  <td>ร้อยละของผลประโยชน์ : </td>
                  <td><input type="number" name="BENEFICIARY_BENEFIT2" maxlength="3" size="8" value="<?php echo $row["BENEFICIARY_BENEFIT2"]; ?>" /></td>
                </tr>
                <tr>
                  <td>เพศ : </td>
            ...
            <div <?php if ($max_beneficiary < 3) {
              ...
              <table id="table-form">
                ...
                <tr>
                  ...
                  <th>&nbsp;</th>
                  <td>ร้อยละของผลประโยชน์ : </td>
                  <td><input type="number" name="BENEFICIARY_BENEFIT3" maxlength="3" size="8" value="<?php echo $row["BENEFICIARY_BENEFIT3"]; ?>" /></td>
                </tr>
                <tr>
                  <td>เพศ : </td>
            ...
            <div <?php if ($max_beneficiary < 4) {
                    echo $hide;
                  } ?>>
              <div>
                <h2>ผู้รับผลประโยชน์ 4</h2>
              </div>
              <table id="table-form">
                <tr>
                  <td>คำนำหน้า : </td>
                  <td>
                    <select name="BENEFICIARY_TITLE4">
                      <option value="<?php echo $row["BENEFICIARY_TITLE4"]; ?>">
                        <?php echo $row["BENEFICIARY_TITLE4"]; ?>
                      </option>
                      <?php
                      $strSQL = "SELECT * FROM t_aig_pancode where type = 'titlename' ORDER BY id ASC";
                      $objQuery = mysqli_query($Conn, $strSQL);
                ...
                <tr>
                  ...
                  <td><select name="BENEFICIARY_RELATIONSHIP4">
                      <option value="<?php echo $row["BENEFICIARY_RELATIONSHIP4"]; ?>">
                        <?php echo $row["BENEFICIARY_RELATIONSHIP4"]; ?>
                      </option>
                      <?php
                      $strSQL = "SELECT * FROM t_aig_pancode where type = 'relationship' ORDER BY id ASC";
                      $objQuery = mysqli_query($Conn, $strSQL);
                  ...
                  <th>&nbsp;</th>
                  <td>ร้อยละของผลประโยชน์ : </td>
                  <td><input type="number" name="BENEFICIARY_BENEFIT4" maxlength="3" size="8" value="<?php echo $row["BENEFICIARY_BENEFIT4"]; ?>" /></td>
                </tr>
                <tr>
                  <td>เพศ : </td>
                  <td><select name="BENEFICIARY_SEX4">
                      <option value="<?php echo $row["BENEFICIARY_SEX4"]; ?>">
                        <?php echo $row["BENEFICIARY_SEX4"]; ?>
                      </option>
                      <option value="ชาย">ชาย</option>
                      <option value="หญิง">หญิง</option>
                    </select></td>
        ...
        <!-- <div>
          <fieldset id="form-content">
            ...
            <div class="table">
              ...
              $CHK_APP_CONSENT_Y = "";
              $CHK_APP_CONSENT_N = "";
              if ($row["APP_CONSENT"] == 'Y') {
                $CHK_APP_CONSENT_Y = 'checked';
              }
              if ($row["APP_CONSENT"] == 'N') {
                $CHK_APP_CONSENT_N = 'checked';
              }
              ?>
              <div style="padding: 10px 50px 10px 50px ;  ">
                <label for="APP_CONSENT_YES"><input type="radio" id="APP_CONSENT_YES" name="APP_CONSENT" value="Y" <?php echo $CHK_APP_CONSENT_Y; ?>>
                  ยินยอมให้เปิดเผยข้อมูล </label><label id="APP_CONSENT-error" class="error" for="APP_CONSENT"></label>
                <BR>
                <label for="APP_CONSENT_NO"><input type="radio" id="APP_CONSENT_NO" name="APP_CONSENT" value="N" <?php echo $CHK_APP_CONSENT_N; ?>>
                  ไม่ยินยอมให้เปิดเผยข้อมูล &nbsp;&nbsp;<span style="color:red">* กรณีลูกค้าไม่ให้ความยินยอมส่งข้อมูลสรรพากร ลดหย่อนภาษี</label></span><br>
              </div>
            ...
            <fieldset id="form-content">
              ...
              <div class="table">
                ...
                <div class="table-content">
                  <div class="table-row" <?php if ($url_billing_payment == '') {
                                            echo $hide;
                                          } ?>>
                    <div class="table-data">Change card number and generate billing</div>
                    <div class="table-data"><button type="button" class="button" id="btnPaymentBilling" onclick="window.open('<?php echo $url_billing_payment; ?>')">Payment</button>
                    </div>
                  </div>
                  <div class="table-row" <?php if ($url_online_payment == '') {
                                            echo $hide;
                                          } ?>>
                    <div class="table-data">Online Payment Gateway </div>
                    <div class="table-data"><button type="button" class="button" id="btnPaymentOnline" onclick="window.open('<?php echo $url_online_payment; ?>')">Payment</button>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <fieldset id="form-content">
            ...
            <table id="table-form">
              <tr>
                ...
                <td><input type="text" name="ACCOUNT_NAME" maxlength="50" size="30" value="<?php echo $row["ACCOUNT_NAME"]; ?>" />
                </td>
                <td>
                  <button type="button" class="button" id="secure_pause">Pause</button>
                  <button type="button" class="button" id="unsecure_pause" style="padding-left=5px;margin-left:10px;">Resume</button>
                  <span style="color:red; font-size:larger;" id="pauseDisplay"></span>
                </td>
              </tr>
              <tr>
                <?php
              ...
              <tr>
                <td>วันหมดอายุบัตร : <span style="color:red">*</span></td>
                <td><input type="text" name="ACCOUNT_EXPIRE" maxlength="5" size="5" value="<?php echo $row["ACCOUNT_EXPIRE"]; ?>" readonly>
                </td>
              </tr>
              <tr>
                <td>Payment Key : <span style="color:red">*</span></td>
                <td>
                  <input type="text" name="payment_key" id="payment_key" size="30" maxlength="100" value="<?php echo $row["payment_key"]; ?>" readonly />
                  <?php
                  $SQL = "SELECT campaign_id,BIN_Number FROM t_aig_bin";
                  $result = mysqli_query($Conn, $SQL) or die("ไม่สามารถเรียกดูข้อมูลได้");
                  $rows = array();
                  while ($rs = mysqli_fetch_array($result)) {
                    $rows[] = $rs;
                  }
                  $binResult = json_encode($rows);
                  ?>
                  <button type="button" class="button" id="btnPaste">Paste</button>
                  <div style="display:none" id="divBin"><?php echo $binResult; ?></div>
                  <label id="payment_key-error" class="error" for="payment_key"></label>
                </td>
              </tr>
            </table>
          </fieldset>
        ...
        <div>
          <fieldset id="form-content">
            ...
            <div class="table">
              ...
              <div class="table-content">
                <?php
                $voice_sql = "select voice_id,create_date from t_call_trans where calllist_id = '" . $row['calllist_id'] . "' ORDER BY create_date";
              ...
              <tr>
                ...
                ?>
                  ...
                  <td>
                    ...
                    <input id="datepicker6" type="text" name="Approved_Date_V" size="10" value="<?php echo $Approved_Date; ?>" />
                  </td>
                <?php } ?>
                <th>&nbsp;</th>
                <?php
                $isGenTPA = "";
                $strSQL = "SELECT 1 AS isGenTPA FROM t_aig_app WHERE TPA_Date is not null AND flag != 3 AND id = $id";
                $objQuery = mysqli_query($Conn, $strSQL);
                while ($objResuut = mysqli_fetch_array($objQuery)) {
                  $isGenTPA = $objResuut["isGenTPA"];
                }
                ?>
                <td>
                  <button type="button" class="button" id="submit" <?php echo $submitFlag; ?> <?php echo ($isGenTPA == "1") ? 'style="display:none"' : ''; ?>>Save</button>
                  <span style="color:red<?php echo ($isGenTPA == "1") ? "" : ";display:none"; ?>">ไม่สามารถแก้ไขข้อมูลได้เนื่องจากได้ทำการออกกรมธรรม์แล้ว</span>
                </td>
              </tr>
            </table>
          </fieldset>
