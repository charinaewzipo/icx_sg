...
$objPHPExcel = $objReader->load("templates/Campaign_Performance_by_lead.xls");
...
$sqlTotalCalllistUpload = "SELECT SUM(total_records) As TotalCalllistUpload FROM t_import_list WHERE import_id IN ($campaign_id_selected_lead_list) AND is_count != 0 ";
...
$sqlTotalCalllistManualAdd = "SELECT COUNT(DISTINCT calllist_id) AS TotalCalllistManualAdd FROM t_calllist WHERE import_id IN (SELECT import_id As TotalCalllist FROM t_import_list WHERE import_id IN ($campaign_id_selected_lead_list) AND is_count = 0 )";
...
$sqlTotalCalllistDNC = "SELECT COUNT(DISTINCT l.calllist_id) AS TotalCalllistDNC FROM t_calllist l LEFT JOIN t_calllist_agent_history h ON l.calllist_id = h.calllist_id WHERE l.status IN (11,12,13,14,15,16,17,18,19) AND l.import_id IN (SELECT import_id As TotalCalllist FROM t_import_list WHERE import_id IN ($campaign_id_selected_lead_list)) AND h.last_wrapup_id IS NULL";
...
$sql_app = " SELECT import_id,campaign_id, COUNT(DISTINCT(CASE WHEN AppStatus NOT IN( 'Follow-doc') THEN id ELSE NULL END)) sale_front,
            ...
            from t_aig_app where (create_date BETWEEN '$startdate2 00:00' and '$enddate2 23:59')  and campaign_id in ($campaign_id_selected) group by import_id";
$result_app =  mysqli_query($Conn, $sql_app) or die("ไม่สามารถเรียกดูข้อมูลได้");
$sale_value = array();
while ($row = mysqli_fetch_array($result_app)) {
    $sale_value[] = $row;
}
$sale_value = array_column($sale_value, null, 'import_id');


$SQL = "SELECT import_id,(SELECT list_name FROM t_import_list WHERE a.import_id = t_import_list.import_id) as list_name,c.campaign_id,campaign_name,campaign_mode,Target_Contact,Target_Response,Target_AARP,Target_TARP,Target_App,
...
JOIN (
    ...
    FROM t_calllist_agent_history WHERE (history_date BETWEEN '$startdate2 00:00:00' AND '$enddate2 23:59:59') AND campaign_id IN ($campaign_id_selected) AND import_id IN (SELECT il.import_id FROM t_import_list il INNER JOIN t_campaign_list cl ON il.import_id = cl.import_id WHERE cl.campaign_id in ($campaign_id_selected) AND il.import_id in ($campaign_id_selected_lead_list) ) GROUP BY calllist_id
...
while ($row = mysqli_fetch_array($result)) {
    ...
    $campaign_name_display = "$campaign_name ($campaign_mode) $list_name";
    ...
    if ($campaign_mode == "Predictive") {
        ...
        $sqlGetCampaingName = "SELECT GROUP_CONCAT(QUOTE(genesys_campaign_name) SEPARATOR ',') AS genesys_campaign_name FROM t_import_list where genesys_campaign_name is not null and  import_id in ($campaign_id_selected_lead_list)";
    ...
    $sqlTotalCalllistUploadPercampaign = "SELECT SUM(total_records) As TotalCalllistUploadPercampaign FROM t_import_list WHERE import_id IN (SELECT import_id FROM t_campaign_list WHERE import_id in ($import_id)) AND is_count != 0 ";
    ...
    $sqlTotalCalllistManualAddPercampaign = "SELECT COUNT(DISTINCT calllist_id) AS TotalCalllistManualAddPercampaign FROM t_calllist WHERE import_id IN (SELECT import_id As TotalCalllist FROM t_import_list WHERE import_id IN (SELECT import_id FROM t_campaign_list WHERE import_id in ($import_id)) AND is_count = 0)";
    ...
    $sqlTotalCalllistDNCPercampaign = "SELECT COUNT(DISTINCT l.calllist_id) AS TotalCalllistDNCPercampaign FROM t_calllist l LEFT JOIN t_calllist_agent_history h ON l.calllist_id = h.calllist_id WHERE l.status IN (11,12,13,14,15,16,17,18,19) AND l.import_id IN (SELECT import_id As TotalCalllist FROM t_import_list WHERE import_id IN (SELECT import_id FROM t_campaign_list WHERE import_id in ($import_id))) AND h.last_wrapup_id IS NULL";
    ...
    // sale_app
    $sale_app_front = (isset($sale_value[$import_id]["sale_front"])) ? $sale_value[$import_id]["sale_front"] : 0;
    $sale_app_back = (isset($sale_value[$import_id]["sale_back"])) ? $sale_value[$import_id]["sale_back"] : 0;

    // sale_premium_y
    $sale_premium_front_y = (isset($sale_value[$import_id]["premium_year_front"])) ? $sale_value[$import_id]["premium_year_front"] : 0;
    $sale_premium_back_y = (isset($sale_value[$import_id]["premium_year_back"])) ? $sale_value[$import_id]["premium_year_back"] : 0;

    //  sale_premium_M
    $sale_premium_front_m = (isset($sale_value[$import_id]["premium_month_front"])) ? $sale_value[$import_id]["premium_month_front"] : 0;
    $sale_premium_back_m = (isset($sale_value[$import_id]["premium_month_back"])) ? $sale_value[$import_id]["premium_month_back"] : 0;
...
$droot = "temp/";
$fname = "Campaign_Performance_by_lead" . $currentdate . ".xls";
