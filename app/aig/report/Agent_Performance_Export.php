...
if($group_id){
    ...
    $result = mysqli_query($Conn, $SQL_groupname) or die ("ไม่สามารถเรียกดูข้อมูลได้");
    while($row = mysqli_fetch_array($result))
        {
...
if($team_id){
    ...
    $result = mysqli_query($Conn, $SQL_teamname) or die ("ไม่สามารถเรียกดูข้อมูลได้");
    while($row = mysqli_fetch_array($result))
        {
...
(
SELECT COUNT(calllist_id) FROM t_call_trans t where (create_date BETWEEN '$startdate2 00:00:00' and '$enddate2 23:59:59') AND t.agent_id = a.agent_id AND t.campaign_id in ($campaign_id_selected)
) AS list_attempt
...
JOIN (
    ...
    FROM $table WHERE ($field_date BETWEEN '$startdate2 00:00:00' AND '$enddate2 23:59:59') AND campaign_id in ($campaign_id_selected) AND import_id IN (SELECT il.import_id FROM t_import_list il INNER JOIN t_campaign_list cl ON il.import_id = cl.import_id WHERE cl.campaign_id in ($campaign_id_selected) AND il.list_comment = DATE_FORMAT('$enddate2','%Y%m')) GROUP BY calllist_id
...
SELECT a.agent_id, CONCAT(first_name,' ',last_name) AS agentname,
0 AS list_use,
0 AS list_success,
0 AS list_unsuccess,
0 AS list_followup,
...
WHERE (a.create_date BETWEEN '$startdate2 00:00' and '$enddate2 23:59') and campaign_id in ($campaign_id_selected) $sql_condition GROUP BY agent_id
) x GROUP BY agent_id
" ;
...
from t_aig_app where (create_date BETWEEN '$startdate2 00:00' and '$enddate2 23:59') and campaign_id in ($campaign_id_selected) group by agent_id";
