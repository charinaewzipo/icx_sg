...
$ProposalNumber = $_GET["Id"];
 ...
 $result = mysql_query($SQL,$Conn) or die ("ไม่สามารถเรียกดูข้อมูลได้");
 	while($row = mysql_fetch_array($result))
	{
			...
			$data['address1'] = $AddressNo1;
			$data['Moo1'] = $Moo1;
			$data['Soi1'] = $Soi1;
			$data['building1'] = $building1;
			$data['Road1'] = $Road1;
			$data['Sub_district1'] = $Sub_district1;
			...
			$data['BENEFICIARY_AGE1'] = $row["BENEFICIARY_BENEFIT1"];
			//$data['BENEFICIARY_RELATIONSHIP1'] = $row["BENEFICIARY_RELATIONSHIP1"];
			$BENEFICIARY_RELATIONSHIP1 = $row["BENEFICIARY_RELATIONSHIP1"];
			if($BENEFICIARY_RELATIONSHIP1 == 'บิดา'){
				$data['BENEFICIARY_RELATIONSHIP1_1'] = 'Yes' ;
			}elseif ($BENEFICIARY_RELATIONSHIP1 == 'มารดา'){
				$data['BENEFICIARY_RELATIONSHIP1_2'] = 'Yes' ;
			}elseif ($BENEFICIARY_RELATIONSHIP1 == 'สามี'){
				$data['BENEFICIARY_RELATIONSHIP1_3'] = 'Yes' ;
			}elseif ($BENEFICIARY_RELATIONSHIP1 == 'ภรรยา'){
				$data['BENEFICIARY_RELATIONSHIP1_4'] = 'Yes' ;
			}elseif ($BENEFICIARY_RELATIONSHIP1 == 'บุตร'){
				$data['BENEFICIARY_RELATIONSHIP1_5'] = 'Yes' ;
			}
			
			
			$data['BENEFICIARY_NAME2'] = $row["BENEFICIARY_TITLE2"].$row["BENEFICIARY_NAME2"].' '.$row["BENEFICIARY_LASTNAME2"] ;
			$data['BENEFICIARY_AGE2'] = $row["BENEFICIARY_BENEFIT2"];
			//$data['BENEFICIARY_RELATIONSHIP2'] = $row["BENEFICIARY_RELATIONSHIP2"];
			
			$BENEFICIARY_RELATIONSHIP2 = $row["BENEFICIARY_RELATIONSHIP2"];
			if($BENEFICIARY_RELATIONSHIP1 == 'บิดา'){
				$data['BENEFICIARY_RELATIONSHIP2_1'] = 'Yes' ;
			}elseif ($BENEFICIARY_RELATIONSHIP2 == 'มารดา'){
				$data['BENEFICIARY_RELATIONSHIP2_2'] = 'Yes' ;
			}elseif ($BENEFICIARY_RELATIONSHIP2 == 'สามี'){
				$data['BENEFICIARY_RELATIONSHIP2_3'] = 'Yes' ;
			}elseif ($BENEFICIARY_RELATIONSHIP2 == 'ภรรยา'){
				$data['BENEFICIARY_RELATIONSHIP2_4'] = 'Yes' ;
			}elseif ($BENEFICIARY_RELATIONSHIP2 == 'บุตร'){
				$data['BENEFICIARY_RELATIONSHIP2_5'] = 'Yes' ;
			}
			...
			$data['COVERAGE_NAME'] = 'Gen Exclusive ('.$plan_code.')';
            ...
            // need to know what file the data will go into
			 $pdf_doc='http://192.168.8.250/tubtim/app/gen/Application/Form/GenPAForm2.pdf';
