...
header('Content-Disposition: attachment; filename="Gen Health Lump Sum.xls"');# ชื่อไฟล์
?>
...
<TD>Policy Owner Nationality</TD>
<TD>Second Insured person Nationality</TD>
<TD>Policy Expiry Date</TD>
<TD>Third Insured person Nationality</TD>
<TD>Blank field</TD>
<TD>ID Number</TD>
<TD>Third Insured - Surname</TD>
<TD>Third Insured - Given name</TD>
<TD>Salutation</TD>
<TD>Salutation code</TD>
<TD>Client Sex</TD>
<TD>Client Sex code</TD>
...
<TD>Client Address 3</TD>
<TD>Client Address 4</TD>
<TD>Client Address 5</TD>
<TD>Post Code</TD>
<TD>Client Telephone Number 1</TD>
<TD>Client Telephone Number 2</TD>
<TD>Client Telephane Number 3</TD>
<TD>e-mail address</TD>
<TD>Client Date of Birth</TD>
...
<TD>Beneficiary age 4</TD>
<TD>Beneficiary benefit % 4</TD>
<TD>Policy Owner - Type of ID Card</TD>
<TD>Policy Owner - ID Card Issue District</TD>
<TD>Policy Owner - ID Card Issue Province</TD>
<TD>Policy Owner - ID Card Issue Country</TD>
<TD>Policy Owner - ID Card Expiry Date</TD>
<TD>Policy Owner - Annual Income</TD>
<TD>Policy Owner - Company Name</TD>
<TD>Second Insured - Type of ID Card</TD>
<TD>Third Insured - Type of ID Card</TD>
<TD>Out patient premium</TD>
<TD>Childbirth premium</TD>
<TD>Accident premium</TD>
<TD>Dental premium</TD>

<TD>ID Number of Payor</TD>
<TD>Surname of Payor</TD>
<TD>Given Name of Payor</TD>
<TD>Payer Date of Birth</TD>
<TD>Payer Title and Occupation Details </TD>
<TD>Payer Occupation Code</TD>
<TD>Payer Age</TD>
<TD>Payer Telphone Number 1</TD>
<TD>Payer Telphone Number 2</TD>
<TD>Payer Salutation</TD>
<TD>Payer Salutation code</TD>
<TD>Payer Relationship to the "Insured"</TD>
<TD>Payer Type of ID Card</TD>
<TD>Tax Refund (Yes/No)</TD>
...
<TD>Payer Sex</TD>
<TD>Payer Sex code</TD>
<TD>Payer status</TD>
<TD>Payer Status code</TD>
<TD>Payer Address 1</TD>
<TD>Payer Address 2</TD>
<TD>Payer Address 3</TD>
<TD>Payer Address 4</TD>
<TD>Payer Address 5</TD>
<TD>Payer Post Code</TD>
<TD>Payer e-mail address</TD>
<TD>Insured Tax Id</TD>
<TD>Payer Tax Id</TD>
...
<?php
$SQL = "select * from `t_app` where campaign_id = '1' and camp = 'GenHealth' and ( Approved_Date between '$startdate' and '$enddate') and AppStatus = 'Success' " ;
...
// ถ้าผู้ชำระเงิน ไม่ใช่ผู้เอาประกัน	
	$payment_relation = $row["payment_relation"];
if($payment_relation != 'ผู้เอาประกัน'){
	$payment_relation = $row["payment_relation"];
	
	$payment_title = $row["payment_title"];
	$SQL2 = "select *  from t_salutation  where `desc` = '$payment_title'" ;
			 $result2 = mysql_query($SQL2,$Conn) or die ("ไม่สามารถเรียกดูข้อมูลได้");
 			while($row2 = mysql_fetch_array($result2))
				{
					$payment_title_code = $row2["code"];	
				}
	
	$payment_name = $row["payment_name"];
	$payment_lastname = $row["payment_lastname"];
	
	$payment_bod = $row["payment_bod"];  // 25/12/1982
	$payment_bod = substr($date, 6, 4).substr($date, 3, 2).substr($date, 0, 2);
	
	$payment_age = $row["payment_age"];
	$payment_idcard = $row["payment_idcard"];
	$payment_phone = $row["payment_phone"];
	
	$payment_occupation = $row["payment_occupation"];
	$SQL2 = "select *  from t_occupation  where `desc` = '$payment_occupation'" ;
			 $result2 = mysql_query($SQL2,$Conn) or die ("ไม่สามารถเรียกดูข้อมูลได้");
 			while($row2 = mysql_fetch_array($result2))
				{
					$payment_occupation_code = $row2["code"];	
				}
	
	
	$payment_postion = $row["payment_postion"];
	$payment_tax = $row["payment_tax"];
	if($payment_tax == 'ไม่ต้องการ'){
		$payment_tax = 'N' ;
	}else{
		$payment_tax = 'Y' ;
	}
	
	$payment_idcard_type = '1';
	
	$Payer_Idcard = $payment_idcard ;
	...
	$Payer_Sex = $row["Payer_Sex"];
	if($Payer_Sex == 'ชาย'){
		$Payer_Sex_code = 'M' ;
	}elseif ($Payer_Sex == 'หญิง'){
		$Payer_Sex_code = 'F' ;
	}else{
		$Payer_Sex_code = '' ;
		}
	...
	if($Payer_status == 'โสด'){
		$Payer_status_code = 'S' ;
	}elseif($Payer_status == 'สมรส'){
		$Payer_status_code = 'M' ;
	}elseif($Payer_status == 'หม้าย'){
		$Payer_status_code = 'W' ;
	}elseif($Payer_status == 'หย่าร้าง'){
		$Payer_status_code = 'D' ;
	}else{
		$Payer_status_code = '' ;
		}

$AddressNo4 = $row["AddressNo4"];
	$building4 = $row["building4"];
	if($building4 != ''){
		$building4 = ' '.$row["building4"];
	}
	...
	$ClientAddress4 = $AddressNo4.$building4.$Moo4 ;
	$ClientAddress2 = $Soi4.$Road4 ;
	
	$province4 = $row["province4"];
			if($province4 == 'กรุงเทพมหานคร'){
				
				$province4 = $row["province4"];
				$Sub_district4 = 'แขวง'.$row["Sub_district4"];
				$district4 = 'เขต'.$row["district4"];
							
			}elseif ($province4 != ''){			
				$province4 = 'จังหวัด'.$row["province4"];
				$Sub_district4 = 'ตำบล'.$row["Sub_district4"];
				$district4 = 'อำเภอ'.$row["district4"];					
			}else{
				$province4 = '';
				$Sub_district4 = '';
				$district4 = '';
...
<tr>
  ...
  <td x:str>$payment_idcard</td>
  <td x:str>$payment_name</td>
  <td x:str>$payment_lastname</td>
  <td x:str>$payment_bod</td>
  <td x:str>$payment_occupation</td>
  <td x:str>$payment_occupation_code</td>
  <td x:str>$payment_age</td>
  <td x:str></td>
  <td x:str>$payment_phone</td>
  <td x:str>$payment_title</td>
  <td x:str>$payment_title_code</td>
  <td x:str>$payment_relation</td>
  <td x:str>$payment_idcard_type</td>
  <td x:str>$payment_tax</td>
  ...
  <td x:str>$Payer_Sex</td>
  <td x:str>$Payer_Sex_code</td>
  <td x:str>$Payer_status</td>
  <td x:str>$Payer_status_code</td>
  <td x:str>$ClientAddress4</td>
  <td x:str>$ClientAddress2</td>
  <td x:str>$Sub_district4</td>
  <td x:str>$district4</td>
  <td x:str>$province4</td>
  <td x:str>$zipcode4</td>
  <td x:str>$Payer_email</td>
  
  <td x:str>$IdCard</td>
  <td x:str>$Payer_Idcard</td>
