...
while( $i < $dif_date)
{
	...
	$SQL_count=  mysql_query(" SELECT sum(INSTALMENT_PREMIUM) from t_app where Approved_Date = '$date_app' and campaign_id = '1' and AppStatus = 'Success'  ");
	...
	// count PA Submit
	$SQL_submit_PA_count=  mysql_query(" SELECT COUNT(id)  from `t_app` where campaign_id = '1'  and  Approved_Date = '$date_app'  and AppStatus <> 'Follow Doc' and camp = 'GenExclusive'   ");
	$SQL_submit_PA_App = mysql_result($SQL_submit_PA_count, 0);
	
	// count HP Submit
	$SQL_submit_HP_count=  mysql_query(" SELECT COUNT(id)  from `t_app` where campaign_id = '1'  and  Approved_Date = '$date_app'  and AppStatus <> 'Follow Doc' and camp = 'GenHealth'   ");
	$SQL_submit_HP_App = mysql_result($SQL_submit_HP_count, 0);
	
	
		///  check TARP Submit
	$SQL = "select INSTALMENT_PREMIUM,camp  from `t_app` where campaign_id = '1'  and  Approved_Date = '$date_app'  and AppStatus <> 'Follow Doc' " ;
	...
	while($dataRow = mysql_fetch_array($result))
{
	...
	// count PA Success
	$SQL_Success_PA_count=  mysql_query(" SELECT COUNT(id)  from `t_app` where campaign_id = '1'  and  Approved_Date = '$date_app'  and AppStatus = 'Success' and camp = 'GenExclusive'   ");
	$SQL_Success_PA_App = mysql_result($SQL_Success_PA_count, 0);
	
	// count PA Success
	$SQL_Success_HP_count=  mysql_query(" SELECT COUNT(id)  from `t_app` where campaign_id = '1'  and  Approved_Date = '$date_app'  and AppStatus = 'Success' and camp = 'GenHealth'   ");
	$SQL_Success_HP_App = mysql_result($SQL_Success_HP_count, 0);
	
	///  check TARP
	$SQL = "select INSTALMENT_PREMIUM,camp  from `t_app` where campaign_id = '1'  and  Approved_Date = '$date_app'  and AppStatus = 'Success' " ;
    $result = mysql_query($SQL,$Conn) or die ("ไม่สามารถเรียกดูข้อมูลได้");
	while($dataRow = mysql_fetch_array($result))
{
	...
	$objPHPExcel->getActiveSheet()->setCellValue('A'.$row2, $strNewDate)
								   ...
								   ->setCellValue('AB'.$row2, $count_TMR)
								   ->setCellValue('AC'.$row2, $AVG_TMR)
...
$droot = "temp/";
$fname = "Daily_Report_".$date_app.".xls";
