...
$objPHPExcel = $objReader->load("templates/DRC_Exclusive_Template2.xls");
	...
	while($dataRow = mysql_fetch_array($result))
{
 			...
 			while($row3 = mysql_fetch_array($result2))
				{
					...
					$premium_payment2 = $row3["premium_payment2"];
				}
	
	$PaymentFrequency = $dataRow["PaymentFrequency"];	
	if($PaymentFrequency == 'รายเดือน'){
		$INSTALMENT_PREMIUM = $INSTALMENT_PREMIUM + $premium_payment2 ;
	}
	...
	$objPHPExcel->getActiveSheet()->setCellValue('A'.$row2, $r)
								  ...
								  ->setCellValue('F'.$row2, $premium)
								   ->setCellValue('K'.$row2, 'โอนเงินสด')
								   ->setCellValue('M'.$row2, $INSTALMENT_PREMIUM)
...
$droot = "temp/";
$fname = "Gen Exclusive_".$currentdate_DCR."_DCR(เงินสด).xls";
